...
def get_model(model_type: str, seq_len: int):
    ...
    if model_type == "local":
        model_config = LocalTransformerConfig(max_seq_len=seq_len, device=device)
        model = get_local_transformer(model_config)
    elif model_type == "mac":
        model_config = MACConfig(d_model=512, seq_len=seq_len, num_tokens=256)
...
def train(
    model_type: Literal["local", "mac"] = "mac",
    log: bool = False,
    ...
    train_loader, val_loader = get_dataloaders(
        batch_size=train_config.batch_size, seq_len=train_config.seq_len
    )
    ...
    _, val_dataset_loader = get_dataloaders(
        batch_size=1, random_sampling=False, seq_len=train_config.seq_len
    )
    val_dataset = []
    ...
    # training loop
    for i in tqdm.tqdm(
        range(train_config.num_batches), mininterval=10.0, desc=f"training {model_type}"
    ):
        model.train()
        ...
        for __ in range(train_config.gradient_accumulate_every):
            ...
            loss = nn.CrossEntropyLoss()(
                logits.reshape(-1, logits.size(-1)), target_seq.reshape(-1)
            )
            loss.backward()
        ...
        if i % train_config.validate_every == 0:
            ...
            with torch.no_grad():
                ...
                val_loss = nn.CrossEntropyLoss()(
                    val_logits.reshape(-1, val_logits.size(-1)), val_target.reshape(-1)
                )
            ...
            with torch.no_grad():
                ...
                if hasattr(model, "generate"):
                    sample = model.generate(
                        seed_seq[None, ...], train_config.generate_length
                    )
                    output_str = decode_tokens(sample[0].cpu().tolist())
                    print(f"Generated text: {output_str}")
                else:
                    print(
                        "Model doesn't have generate method - skipping text generation"
                    )
                print("*" * 100)
