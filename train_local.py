...
class TrainingConfig:
    """Configuration for training the LocalTransformer model."""
...
for i in tqdm.tqdm(range(train_config.num_batches), mininterval=10., desc='training'):
    model.train()

    for __ in range(train_config.gradient_accumulate_every):
        batch = next(train_loader)
        input_seq = batch[:, :-1]  # Shape: (batch_size, seq_len)
        target_seq = batch[:, 1:]  # Shape: (batch_size, seq_len)
